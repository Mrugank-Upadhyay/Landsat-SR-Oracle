...
export function calculateAcquisitionTimes(
  ...
  function calculateTime(cycle: LandsatCycle) {
    ...
    seconds +=
      correction_offset[cycle.satellite as keyof typeof correction_offset];
    return addSeconds(coverage_start_day_date, seconds);
  }

  function search(
    limit: number,
    increment: (offset: number) => number,
    compare_date: (d: Date) => boolean,
    status: (cycle: LandsatCycle) => string
  ) {
    const results = [];
